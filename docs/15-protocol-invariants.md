...
This document defines the core safety invariants of the UdonFi V2 protocol. These invariants must hold true across all smart contract states, transaction updates, and database updates.
...
- **Applies to**: `price_oracle_aggregator`.
- **How to test**: Mock primary/secondary price feeds with 5% deviation; assert get_price reverts.
- **Severity**: High
...
- **Applies to**: `price_oracle_aggregator`.
- **How to test**: Trigger circuit breaker and query price twice while mocking changing feed inputs; assert returned price remains identical.
- **Severity**: High
...
- **Applies to**: `indexer` sync engine.
- **How to test**: Process the same event XDR log twice; check database record counts to verify no duplicate is inserted.
- **Severity**: High

### INV-IDX-002: Metadata Freshness Check
- **Description**: Every REST API response must include current processed ledger heights.
- **Applies to**: `backend` routing controllers.
- **How to test**: Query endpoints; assert that the `meta` block contains valid ledger integers.
- **Severity**: Medium

### INV-IDX-003: No Stale Flag Bypass
- **Description**: The API must mark `isStale: true` in metadata if database processed ledger lag exceeds 3 blocks.
- **Applies to**: `backend` sync middleware.
- **How to test**: Mock ledger tip to exceed DB sequence height by 5 blocks; assert response reports `isStale: true`.
- **Severity**: High

### INV-IDX-004: Sequential Database Sync
- **Description**: Database ledger sequence height must increase sequentially: `sequence(t) = sequence(t-1) + 1` during index scans.
- **Applies to**: `indexer` pipeline.
- **How to test**: Assert indexer checks sequence ranges and throws if any blocks are skipped.
- **Severity**: High

### INV-IDX-005: Degraded Write Rejections
- **Description**: When sync lag exceeds 10 blocks, dashboard write simulations must reject requests.
- **Applies to**: `backend` API gateway.
- **How to test**: Mock lag to 11 blocks and send borrow simulation; assert API rejects request with sync lag error.
- **Severity**: High
