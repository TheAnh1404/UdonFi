...
These flows describe the current MVP path: frontend, Freighter, Soroban RPC, smart contracts, Stellar Testnet, and Stellar Expert links. No backend, event indexer, or bot is required.
...
Automated liquidation bots, event-indexed liquidation candidate lists, real-time dashboards, backend APIs, queues, and sync-lag strategies are not part of the MVP flow.
