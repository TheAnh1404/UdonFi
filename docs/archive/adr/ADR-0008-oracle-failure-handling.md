...
*   **Status**: Approved for full protocol design; partial MVP implementation only
*   **MVP Scope Refactor**: Full multi-oracle aggregation and off-chain TWAP indexing are not required for the MVP demo. The MVP may use configured testnet/mock price inputs for Health Factor and manual liquidation testing.
...
*   **Decision**: We implement an **Oracle Failure Handling Strategy** in the `price_oracle_aggregator` contract:
    ...
    4. **Fallback Mechanism (TWAP)**: If the primary oracle fails or returns stale data:
       ...
       - Post-MVP, if both fail, it may use a 12-hour Time-Weighted Average Price (TWAP) compiled off-chain by an indexer.
