...
*   **MVP Scope Refactor**: The current MVP does not require an off-chain event indexer. The frontend reads directly from Soroban RPC and writes through Freighter-signed transactions. This ADR is preserved for future analytics/indexer work.
...
*   **Decision**: Post-MVP, we may implement an **Event-Driven Architecture** utilizing an off-chain event indexer to poll and sync ledger events.
*   **Consequences**:
